...
The thesis intended to use IOZone for filesystem benchmarking for all the filesystems to have an unified method of benchmarking. However, it was found that IOZone crashed when performing certain tests on GCSF, such as backwards read. A similar problem was found during development of FFS, that IOZone could not complete certain tests and thus crashed. The problem for FFS was solved, but as of writing, it is not possible to perform certain IOZone tests on GCSF. Instead, a custom test suit has been developed for the comparison FFS and GCSF, TFG. 

Further, it was discovered that FFS also is not suitable for IOZone benchmarking due to API rate limits
