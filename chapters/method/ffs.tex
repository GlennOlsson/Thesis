...
\begin{table}[!ht]
	\begin{center}
		\caption{Filesystem operations implementable trough the \gls{FUSE} API, and wether or not \gls{FFS} implements them}
		\begin{tabular}{| c | c |}
...
While the Flickr API is extensive in its functionality, \gls{FFS} only uses a few of the provided capabilities. The Flickr API capabilities that \gls{FFS} uses are:
\begin{itemize}
...
\begin{itemize}
	...
	\item query the Flickr API for the \gls{URL} pointing to the most recently uploaded image,
...
Another limitation of \gls{FFS} is the rate limits presented by the Flickr API. Flickr allows up to $3\,600$ API requests per hour, after which the API keys may be revoked by Flickr. $3\,600$ requests per hour equals $60$ requests per minute, or $1$ request per second. If the average request takes less than a second for constant, sequential Flickr API calls, the API keys could be revoked. Further, some requests are sent concurrently to Flickr which means that \gls{FFS} could reach $3\,600$ API calls faster. However, as long as \gls{FFS} is not constantly serving filesystem calls, the API limit should be of little concern.
