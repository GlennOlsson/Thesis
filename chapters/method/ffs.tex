...
\subsection{Cache}
\gls{FFS} implements a simple in-memory \gls{LRU} cache for the downloaded content. The cache consists of two data structures: 
\begin{description}
...
\gls{FFS} separately caches both the root directory and the inode table. As both of these data structures are used in many of the filesystem operations, it is important that they can be accessed quickly and not be removed from the cache. Their cache entries are updated when the files are uploaded to the \gls{OWS}.

\gls{FFS} separately also caches the inode and the parent directory of open files, and the file's data if it has have been modified. A file is opened with the use of the \texttt{open} or \texttt{create} filesystem operation, as described further down in Section~\ref{subsec:file_op}. When a file is opened it is associated with a file handle identifier which is used for subsequent filesystem operations to refer to the file rather than using the path to the file in the filesystem. When a user is writing data to a file, multiple \texttt{write} file operations might be executed. For instance, when writing a \SI{100}{\byte} file two \texttt{write} operations might be executed:
\begin{itemize}
...
\end{itemize}
To save computation time by not having to download the file form the \gls{OWS}, or even decrypt the image data found in \gls{FFS}s regular cache, \gls{FFS} stores the modified file data separately in memory. When a file with a file handle is modified or read, \gls{FFS} checks if the file handle has any modified data associated with it, before it is checking the regular cache. If there is data associated with the file handle, then this data is used for the file operation. If the file operation was a modifying operation, such as a \texttt{write} operation, the new data is associated with the file handle and stored in memory. When the file is closed with a subsequent \texttt{close} file operation, the modified data is encoded, encrypted and uploaded to the \gls{OWS}. There is no limit to how many files can be open in the filesystem at the same time, nor how much modified data can be associated with a file handle. Further, if a file is not closed, the associated data is not disassociated with the file handle and is kept in the memory until the filesystem is shut down.
...
% 	"Can you quantify this? if thee are 4 API calls for a file system operation such as read (as per §4.2.4) - this mean 15 such operations per minute. What are the implications of this for the depth of the directory tree?" for paragraph about API limits
